...
import { canViewSubitem, findFile, forbidden, hasFullBoardAccess } from "@/lib/server/access";
...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  if (located.file.parentType === "project") {
    const canViewProjectFile = hasFullBoardAccess(user) || located.project.ownerId === user.id;

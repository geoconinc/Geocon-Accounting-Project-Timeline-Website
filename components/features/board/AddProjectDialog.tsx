...
export function AddProjectDialog({
  ...
  // Portal to body so sticky board headers (z-20 inside overflow scroll) cannot cover the dialog.
  return createPortal(
    <div
    ...
    >
      <div
        className="bg-white rounded-lg shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto animate-fade-in-up"
        ...
        <div className="flex flex-col gap-3">
          ...
          <label className="flex flex-col gap-1">
            ...
            {pmUsers.length === 0 && (
              <span className="text-[10px] text-amber-700">
                No PMs matched yet — refresh the page after sign-in to load the roster from JSON.
              </span>
            )}
          </label>
          <label className="flex flex-col gap-1">
          ...
          {directorsMissing.length > 0 && (
            <p className="text-[10px] text-amber-800 bg-amber-50 border border-amber-100 rounded px-2 py-1.5">
              ...
              {directorsMissing.map((d) => d.chartLabel).join(", ")}.
            </p>
          )}
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button
              type="button"
              onClick={() => void submit()}
              disabled={busy || !name.trim() || !projectManagerId}
